class BlogManager {
  constructor() {
    ...
    this.nextId = 1;  // Unique ID generator for each blog
  }

  // Reset state function to clear posts and reset nextId
  resetState() {
    this.nextId = 1;  // Reset the ID counter
    this.posts.length = 0;  // Clear the posts array
    console.log("State reset. posts:", this.posts);
  }

  // Add a new blog post
  addPost(title, authorName, authorEmail, content, tags) {
    const postId = this.nextId++;  // Increment and assign the next ID
...
module.exports = {
  addPost: blogManager.addPost,
  getPost: blogManager.getPost,
  updatePost: blogManager.updatePost,
  deletePost: blogManager.deletePost,
  resetState: blogManager.resetState,
  blogArray: blogManager.posts,
};
